...
export default function BudgetApp() {
  ...
  useEffect(() => { (async () => { try { const r = await window.storage.get("budget-app-v3"); if (r && r.value) { const d = JSON.parse(r.value); if (d.items) setItems(d.items); if (d.goals) setGoals(d.goals); if (d.view) setView(d.view); if (d.darkMode !== undefined) setDarkMode(d.darkMode); } } catch(e){} setLoaded(true); })(); }, []);
  useEffect(() => { if (!loaded) return; (async () => { try { await window.storage.set("budget-app-v3", JSON.stringify({ items, goals, view, darkMode })); } catch(e){} })(); }, [items, goals, view, darkMode, loaded]);
      ...
      <div style={{ padding: "16px 20px 120px" }}>
        ...
        {activeTab === "dashboard" && (items.length === 0 ? (
          ...
          <>
            ...
            <button onClick={async () => { setItems([]); setGoals([]); try { await window.storage.delete("budget-app-v3"); } catch(e){} }} style={{ width: "100%", padding: "12px", background: T.inputBg, border: `1px solid ${T.inputBorder}`, borderRadius: "10px", color: T.textLight, fontSize: "12px", cursor: "pointer", fontFamily: "inherit" }}>Clear All Data</button>
          </>
        ))}
